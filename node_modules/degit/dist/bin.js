...
var index = require('./index-b4afe6fe.js');
var os = require('os');
...
var symbols$1 = index.createCommonjsModule(function (module) {
...
keypress.listen = (options = {}, onKeypress) => {
  ...
  let rl = readline__default['default'].createInterface({ terminal: true, input: stdin });
...
symbols.merge = options => {
  let result = utils.merge({}, ansiColors.symbols, symbols, options.symbols);
  delete result.merge;
  return result;
};
...
class Prompt extends Events__default['default'] {
  constructor(options = {}) {
    super();
...
const factory$1 = (authenticate = defaultAuthenticate$1) => {
  ...
  class AuthPrompt extends form$1 {
    constructor(options) {
      super(options);
    }
...
var boolean = BooleanPrompt;

class ConfirmPrompt extends boolean {
  constructor(options) {
    super(options);
...
class TogglePrompt extends boolean {
  async initialize() {
    await super.initialize();
...
var prompts$1 = index.createCommonjsModule(function (module, exports) {
...
class Enquirer extends Events__default['default'] {
  ...
  register(type, fn) {
    ...
    assert__default['default'].equal(typeof fn, 'function', 'expected a function');
  ...
  async ask(question) {
    ...
    assert__default['default'](this.prompts[type], `Prompt "${type}" is not registered`);
  ...
  static get prompt() {
    ...
    utils.mixinEmitter(fn, new Events__default['default']());
    return fn;
  }
}

utils.mixinEmitter(Enquirer, new Events__default['default']());
...
async function main(argv) {
	...
	if (args.help) {
		const help = fs__default['default']
			.readFileSync(path__default['default'].join(__dirname, '..', 'help.md'), 'utf8')
			.replace(/^(\s*)#+ (.+)/gm, (m, s, _) => s + index.source.bold(_))
			.replace(/_([^_]+)_/g, (m, _) => index.source.underline(_))
			.replace(/`([^`]+)`/g, (m, _) => index.source.cyan(_));
		...
		sync(`**/access.json`, { cwd: index.base }).forEach((file) => {
			const [host, user, repo] = file.split(path__default['default'].sep);

			const json = fs__default['default'].readFileSync(`${index.base}/${file}`, 'utf8');
		...
		const getChoice = (file) => {
			...
			return Object.entries(index.tryRequire(`${index.base}/${file}`)).map(([ref, hash]) => ({
		...
		const empty = !fs__default['default'].existsSync(options.dest) || fs__default['default'].readdirSync(options.dest).length === 0;
		...
		if (!empty) {
			...
			if (!force) {
				console.error(index.source.magenta(`! Directory not empty — aborting`));
				return;
			}
		}
...
function run(src, dest, args) {
	...
	d.on('info', (event) => {
		console.error(index.source.cyan(`> ${event.message.replace('options.', '--')}`));
	});

	d.on('warn', (event) => {
		console.error(index.source.magenta(`! ${event.message.replace('options.', '--')}`));
	});

	d.clone(dest).catch((error) => {
		console.error(index.source.red(`! ${error.message.replace('options.', '--')}`));
		process.exit(1);
	});
}
