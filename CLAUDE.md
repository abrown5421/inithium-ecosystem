...
### Dark Mode & the Surface-Foreground Contract
Dark mode is implemented entirely as a CSS variable flip, not a second set of components or a `dark:` utility variant sprinkled through the codebase. `theme.css` defines a `:root[data-theme='dark']` block that inverts only the `--ui-surface-*` and `--ui-surface-foreground-*` scales (mirrored around their midpoint: `100↔950`, `200↔900`, `300↔800`, `400↔700`, `500↔600`) — the 5 raw brand colors and their own `-foreground` partners are deliberately **not** touched, since they carry brand identity, not layering intent, and must look identical in both themes. `apps/web/src/app/RootRouter.tsx` is the single place that resolves the effective boolean (the `appearance.darkModeEnabled` admin setting gating the signed-in user's own `UserEntity.darkMode` preference — see `libs/db/src/contracts/user.contract.ts` and `libs/cms/src/settings/definitions/dark-mode-enabled.setting.ts`) and stamps `data-theme` onto `<html>`. It lives there specifically because it's the one component mounted on every route, public site and `/cms` alike — never duplicate that effect inside `App`, `CmsRoot`, or any other route-scoped component; extend the existing one instead.

Because of that mechanism, **any color a viewer needs to read correctly in both themes must be expressed as a `surface`/`surface-foreground` token** — never a hardcoded hex, an unset/inherited color, or a raw Tailwind gray/black/white (`text-black`, `text-gray-900`, `#0a0a0a`, ...). Concretely:
- Primary/body text and headings: `textColor={{ color: 'surface', intensity: 950 }}` (or the literal `text-surface-950` class). In light mode this reads as near-black; in dark mode the identical class reads as near-white, because the underlying CSS variable — not the class — is what flips.
- Muted/secondary text: a lighter surface intensity (`500`–`700` is the existing convention across the CMS and public site) — still on the `surface` scale, still auto-inverting.
- **Never leave a `<Text>`, heading, or other text-bearing element without an explicit color.** An element with no `textColor` prop and no `text-*` class in its `className` falls through to the browser's literal default (black), which is wired to no CSS variable and will not invert — this exact bug shipped across several CMS screens (page headings, list-row primary labels) before being caught and fixed. Every new text element needs a deliberate `surface`/`surface-foreground` (or brand) token, never an implicit default.
- Only `100, 200, 300, ..., 950` (step 100) are real, safelisted intensities. A value outside that set (`surface-50`, `surface-1000`, ...) does not fail loudly — it silently produces a class Tailwind never generates, so the element falls back to the exact same unstyled-black failure mode as an unset color. This precise typo (`text-surface-1000`) shipped in three shared `libs/ui` components before being caught. Check any hand-typed intensity against `COLOR_INTENSITIES` in `libs/ui/src/contracts/color.contract.ts`.
- Brand colors (`primary`, `secondary`, `accent`, `tertiary`, `quaternary`, and their `-foreground` partners) are exempt from this contract by design — they don't invert, and code should never assume they will.

When adding or touching any screen, component, or CMS module: audit every text and background color it introduces against this contract before considering the work done — this is a mandatory finishing step, not an optional polish pass, exactly like the Postman collection update required for a new Core API route (section 6).
